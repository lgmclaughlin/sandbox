...
iptables -A OUTPUT  -m set --match-set git-domains dst -j NFLOG --nflog-group 1 --nflog-prefix "SBX_ALLOW" 2>/dev/null || true
iptables -A OUTPUT  -m set --match-set git-domains dst -j ACCEPT
iptables -A OUTPUT  -m set --match-set allowed-domains dst -j NFLOG --nflog-group 1 --nflog-prefix "SBX_ALLOW" 2>/dev/null || true
iptables -A OUTPUT  -m set --match-set allowed-domains dst -j ACCEPT

iptables -A OUTPUT -j NFLOG --nflog-group 2 --nflog-prefix "SBX_BLOCK" 2>/dev/null || true
